...
namespace MaiLib
{
    public class SimaiScanner
    {
        ...
        public char CurrentChar { get => IncomingChart is null || LineNum >= IncomingChart.Length || CharNum >= IncomingChart[LineNum].Length ? ' ' : IncomingChart[LineNum][CharNum]; }
        ...
        public void ScanAndAdvance()
        {
            ...
            {
                if (CharNum < IncomingChart[LineNum].Length) switch (IncomingChart[LineNum][CharNum])
                {
                    case '(':
